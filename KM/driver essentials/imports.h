...
#include <ntimage.h>
#include <xtr1common>
#include "../driver methods/includes/callstack.h"
...
#pragma pack(pop)
	...
	{
		SPOOF_FUNC;
		if (!struc::kernel_base)
			struc::kernel_base = (uintptr_t)struc::ntos_image_base;

		const auto dos_header = (detail::PIMAGE_DOS_HEADER)struc::kernel_base;
		const auto nt_headers = (detail::PIMAGE_NT_HEADERS64)(struc::kernel_base + dos_header->e_lfanew);
		const auto export_directory = (detail::PIMAGE_EXPORT_DIRECTORY)(struc::kernel_base +
			nt_headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress);

		const auto address_of_functions = (UINT32*)(struc::kernel_base + export_directory->AddressOfFunctions);
		const auto address_of_names = (UINT32*)(struc::kernel_base + export_directory->AddressOfNames);
		const auto address_of_name_ordinals = (UINT16*)(struc::kernel_base + export_directory->AddressOfNameOrdinals);

		for (UINT32 i = 0; i < export_directory->NumberOfNames; ++i)
		{
			const auto export_entry_name = (char*)(struc::kernel_base + address_of_names[i]);
			const auto export_entry_hash = KLI_HASH_RTS(export_entry_name);

			// kys
			if (export_entry_hash == ExportHash)
			{
				return struc::kernel_base + address_of_functions[address_of_name_ordinals[i]];
			}
		}
